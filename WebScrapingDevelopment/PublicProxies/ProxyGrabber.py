...
# Configuration
GOOD_PROXIES_FILE = "../_good_proxies.txt"
BAD_PROXIES_FILE = "../_bad_proxies.txt"
VALIDATE_IMPLICIT_WAIT = 5
VALIDATE_DELAY = 5
VALIDATE_THREADS = 7
PROXY_VALIDATE_LIMIT = 200

PROXY_NOVA_IMPLICIT_WAIT = 5
PROXY_NOVA_DELAY = 15
...
def get_proxies_proxy_nova(proxyList, valid_starters):
    ...
    for proxy in valid_starters:
        ...
        driver.implicitly_wait(PROXY_NOVA_IMPLICIT_WAIT)
        
        try:
            driver.get("https://www.proxynova.com/proxy-server-list/")
            time.sleep(PROXY_NOVA_DELAY)
...
def check_proxies():
    ...
    while not q.empty():
        ...
        try:
            ...
            driver = Chrome(options=options)
            driver.implicitly_wait(VALIDATE_IMPLICIT_WAIT)
...
def validate_proxies(proxy_list_raw, fast = False):
    ...
    for p in proxy_list_raw:
        if counter >= PROXY_VALIDATE_LIMIT:
            break
        q.put(p)
        counter += 1
...
def get_public_proxies(fast = False):
    proxyList = []
    validProxies = []
    proxyStarters = []
