...
def set_clipboard(string):
    if sys.platform == "win32":
        ...
        newStr = NSString.stringWithString_(string)
...
# Open the inspect window for the first tab
if sys.platform == "win32":
...
# Create the tabs (include the tab already opened)
for index in range(0, TABS - 1):
    # Create the tab
    if sys.platform == "win32":
        keyboard.press(Key.ctrl)
    elif sys.platform == "darwin":
...
for index in range(0, len(parseList), TABS):
    for tab in range(0, TABS):
        # Check if there are remaining products in the parseList
        if index + tab < len(parseList):
            # Switch to tab
            if tab == 0:
                if sys.platform == "win32":
                    keyboard.press(Key.ctrl)
    ...
    # Space out the loading between the tabs
    for tab in range(0, TABS):
        # Switch to tab
        if tab == 0:
            if sys.platform == "win32":
    ...
    for tab in range(0, TABS):
        ...
        if index + tab < len(parseList):
            ...
            if lastSpaceIndex >= 0:
                ...
                try:
                    if html is None:
                        print("\033[93m" + "Warning: HTML was none. Page potentially did not load fast enough." + "\033[0m")
